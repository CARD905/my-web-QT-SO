...
function Card({ children, className }: { children:React.ReactNode; className?:string }) {
  return <div className={className} style={{ background:"var(--bg-card)", borderRadius:"14px", border:"1px solid var(--border)", padding:"24px", backdropFilter:"blur(12px)", boxShadow:"var(--shadow-card)" }}>{children}</div>;
}
function InfoField({ label, children }: { label:string; children:React.ReactNode }) {
...
function SectionTitle({ icon, children }: { icon:React.ReactNode; children:React.ReactNode }) {
  return <h2 style={{ fontFamily:"var(--font-display)", fontSize:"10.5px", fontWeight:700, color:"var(--text-muted)", letterSpacing:"0.1em", marginBottom:"20px", display:"flex", alignItems:"center", gap:"8px" }}>{icon}{children}</h2>;
}
function ActionBtn({ onClick, variant, children }: { onClick:()=>void; variant:string; children:React.ReactNode }) {
  const styles: Record<string,React.CSSProperties> = {
    outline:{ background:"var(--bg-card-hover)", color:"var(--text-primary)", border:"1px solid var(--border)" },
  ...
  return (
    <button onClick={onClick} style={{ display:"flex", alignItems:"center", gap:"6px", padding:"8px 16px", borderRadius:"10px", fontSize:"13px", fontWeight:700, fontFamily:"var(--font-body)", cursor:"pointer", transition:"all 0.2s ease", ...styles[variant] }}
      onMouseEnter={e=>(e.currentTarget as HTMLElement).style.transform="translateY(-1px)"}
      onMouseLeave={e=>(e.currentTarget as HTMLElement).style.transform="translateY(0)"}
    >{children}</button>
  );
...
const statusMap: Record<string,{label_en:string;label_th:string;bg:string;color:string;dot:string}> = {
...
export default function DetailPage() {
  ...
  const stRaw = statusMap[data.status]??statusMap["draft"];
  const st = { ...stRaw, label: lang==="th" ? stRaw.label_th : stRaw.label_en };
  ...
  return (
    <SidebarLayout>
      <div style={{padding:"32px 36px 60px",maxWidth:"960px"}}>
        ...
        <Card className="fade-up">
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{display:"flex",alignItems:"center",gap:"14px"}}>
              ...
              <div>
                <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
                  <h1 style={{fontFamily:"var(--font-mono)",fontSize:"18px",fontWeight:700,color:"var(--text-primary)",letterSpacing:"0.01em"}}>{data.quotation_no}</h1>
                  <span style={{display:"inline-flex",alignItems:"center",gap:"5px",background:st.bg,color:st.color,padding:"4px 11px",borderRadius:"20px",fontSize:"11.5px",fontWeight:700,letterSpacing:"0.04em",border:`1px solid ${st.color}30`}}>
                    <span style={{width:"5px",height:"5px",borderRadius:"50%",background:st.dot,boxShadow:`0 0 6px ${st.dot}`}}/>
                    {st.label}
                  </span>
                </div>
                <p style={{fontSize:"12.5px",color:"var(--text-muted)",marginTop:"2px"}}>{t("quotationDetails")}</p>
              </div>
            </div>
        ...
        <div className="fade-up fade-up-1" style={{marginTop:"16px"}}>
          <Card>
            <SectionTitle icon={<svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="var(--accent)" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414A1 1 0 0121 9.414V19a2 2 0 01-2 2z"/></svg>}>{t("docDetails")}</SectionTitle>
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"24px"}}>
              <InfoField label={t("docNumber")}>{data.quotation_no}</InfoField>
              <InfoField label={t("issueDate")}>{data.issue_date}</InfoField>
              <InfoField label={t("expiryDate")}>{data.expiry_date}</InfoField>
              <div>
                <p style={{fontSize:"11px",fontWeight:600,color:"var(--text-muted)",letterSpacing:"0.06em",marginBottom:"6px"}}>{t("status").toUpperCase()}</p>
                <span style={{display:"inline-flex",alignItems:"center",gap:"5px",background:st.bg,color:st.color,padding:"4px 10px",borderRadius:"20px",fontSize:"11.5px",fontWeight:700,border:`1px solid ${st.color}30`}}>
                  <span style={{width:"5px",height:"5px",borderRadius:"50%",background:st.dot,boxShadow:`0 0 6px ${st.dot}`}}/>{st.label}
                </span>
              </div>
            </div>
          </Card>
        ...
        <div className="fade-up fade-up-2" style={{marginTop:"16px"}}>
          <Card>
            <SectionTitle icon={<svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="var(--accent)" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>}>{t("customerInfo").toUpperCase()}</SectionTitle>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"20px 48px"}}>
              {[{label:t("name"),value:data.customer_name},{label:t("company"),value:data.customer_company},{label:t("email"),value:data.customer_email},{label:t("phone"),value:data.customer_phone},{label:t("billingAddr"),value:data.customer_address},{label:t("shippingAddr"),value:data.customer_address}].map(f=>(
                <InfoField key={f.label} label={f.label}>{f.value||"—"}</InfoField>
              ))}
            </div>
          </Card>
        ...
        <div className="fade-up fade-up-2" style={{marginTop:"16px"}}>
          <Card>
            <SectionTitle icon={<svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="var(--accent)" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>}>{t("items")}</SectionTitle>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:"13.5px"}}>
              <thead>
                <tr style={{borderBottom:"1px solid var(--border)"}}>
                  {[t("product"),t("description"),t("qty"),t("unitPrice"),t("discount"),t("lineTotal")].map((h,i)=>(
                ...
                {data.items.map((item:any,idx:number)=>{
                  ...
                  return (
                    <tr key={idx} style={{borderBottom:idx<data.items.length-1?"1px solid var(--border)":"none"}}>
                      ...
                      <td style={{padding:"14px 12px",textAlign:"right",color:"var(--text-secondary)"}}>{item.qty}</td>
                      <td style={{padding:"14px 12px",textAlign:"right",color:"var(--text-secondary)",fontFamily:"var(--font-mono)"}}>{money(item.unit_price)}</td>
                      <td style={{padding:"14px 12px",textAlign:"right",color:"#f87171",fontWeight:600}}>{item.discount_percent?`-${item.discount_percent}%`:money(discountAmt)}</td>
                      <td style={{padding:"14px 12px",textAlign:"right",fontWeight:700,color:"var(--text-primary)",fontFamily:"var(--font-mono)"}}>{money(lineTotal)}</td>
                    </tr>
                  );
                })}
        ...
        <div className="fade-up fade-up-3" style={{marginTop:"16px"}}>
          <Card>
            <SectionTitle icon={<svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="var(--accent)" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>}>{t("summary")}</SectionTitle>
            <div style={{display:"flex",justifyContent:"flex-end"}}>
              <div style={{width:"280px"}}>
                {[{label:t("subtotal"),value:money(subtotal+totalDiscount),color:"var(--text-secondary)"},{label:t("discountRow"),value:`-${money(totalDiscount)}`,color:"#f87171"},{label:t("vat"),value:money(vat),color:"var(--text-secondary)"}].map(row=>(
                  <div key={row.label} style={{display:"flex",justifyContent:"space-between",marginBottom:"10px"}}>
                    <span style={{fontSize:"13.5px",color:"var(--text-secondary)"}}>{row.label}</span>
                    <span style={{fontSize:"13.5px",color:row.color,fontWeight:600,fontFamily:"var(--font-mono)"}}>{row.value}</span>
                  </div>
                ...
                <div style={{borderTop:"1px solid var(--border)",paddingTop:"12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{fontSize:"14px",fontWeight:800,fontFamily:"var(--font-display)",color:"var(--text-primary)"}}>{t("grandTotal")}</span>
