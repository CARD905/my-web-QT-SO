...
const focus = (e: React.FocusEvent<any>) => { e.target.style.borderColor="var(--border-hover)"; e.target.style.boxShadow="0 0 0 3px var(--accent-glow)"; };
const blur  = (e: React.FocusEvent<any>) => { e.target.style.borderColor="var(--border)"; e.target.style.boxShadow="none"; };
...
function CustomerDropdown({ customers, selected, onSelect, placeholder }: { customers:CustomerDB[]; selected:number|null; onSelect:(id:number)=>void; placeholder:string }) {
  ...
  return (
    <div ref={ref} style={{ position:"relative" }}>
      ...
      >
        <span style={{ color: selectedC ? "var(--text-primary)" : "var(--text-muted)", fontSize:"13.5px" }}>
          {selectedC ? `${selectedC.customer_name}${selectedC.customer_company ? ` — ${selectedC.customer_company}` : ""}` : placeholder}
        </span>
      ...
      <div style={{
        ...
        opacity: open ? 1 : 0, transform: open ? "translateY(0) scaleY(1)" : "translateY(-6px) scaleY(0.96)",
        transformOrigin:"top", transition:"opacity 0.18s ease, transform 0.18s ease",
...
export default function CreatePage() {
  ...
  const submit = async () => {
    setSubmitting(true);
    try {
      const res = await fetch(`${API}/quotations`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({customer_id:selectedCustomerId,customer,issue_date:date,expiry_date:expiry,items})});
      const data = await res.json();
      if(!res.ok){alert(data.error||"error");return;}
      alert(t("saving").replace("...","") + " ✅");
      router.push("/");
      ...
      <div style={{padding:"32px 36px 60px"}}>
        ...
        <div className="fade-up" style={{marginBottom:"28px"}}>
          <div style={{display:"flex",alignItems:"center",gap:"6px",marginBottom:"10px"}}>
            ...
            <span style={{color:"var(--border)",fontSize:"12px"}}>/</span>
            <span style={{color:"var(--text-secondary)",fontSize:"12.5px"}}>{t("newQ")}</span>
            <span style={{color:"var(--border)",fontSize:"12px"}}>/</span>
            <span style={{color:"var(--text-secondary)",fontSize:"12.5px"}}>{t("edit")}</span>
          </div>
          <h1 style={{fontFamily:"var(--font-display)",fontSize:"28px",fontWeight:800,letterSpacing:"-0.04em",color:"var(--text-primary)"}}>{t("createQuotation")}</h1>
        </div>

        {/* DATE BAR */}
        <div className="fade-up fade-up-1" style={{background:"var(--bg-card)",borderRadius:"14px",border:"1px solid var(--border)",padding:"18px 24px",marginBottom:"20px",display:"flex",justifyContent:"space-between",alignItems:"center",backdropFilter:"blur(12px)"}}>
          <div>
          ...
          <div style={{display:"flex",gap:"12px"}}>
            {[{label:t("issueDate"),value:date,onChange:setDate},{label:t("expiryDate"),value:expiry,onChange:setExpiry}].map(f=>(
              <div key={f.label}>
                ...
                <input type="date" value={f.value} onChange={e=>f.onChange(e.target.value)} style={{...inputStyle,width:"auto",padding:"8px 12px"}} onFocus={focus} onBlur={blur}/>
              </div>
            ))}
          </div>
        </div>
            ...
            <div className="fade-up fade-up-1" style={{background:"var(--bg-card)",borderRadius:"14px",border:"1px solid var(--border)",padding:"24px",backdropFilter:"blur(12px)",boxShadow:"var(--shadow-card)"}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:"14px"}}>
                ...
                <Field label={t("customerName")}>
                  <input placeholder="Full name" value={customer.customer_name} onChange={e=>setCustomer({...customer,customer_name:e.target.value})} style={inputStyle} onFocus={focus} onBlur={blur}/>
                </Field>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px"}}>
                  <Field label={t("company")}><input placeholder="Company name" value={customer.customer_company} onChange={e=>setCustomer({...customer,customer_company:e.target.value})} style={inputStyle} onFocus={focus} onBlur={blur}/></Field>
                  <Field label={t("phone")}><input placeholder="+1 000 000 0000" value={customer.customer_phone} onChange={e=>setCustomer({...customer,customer_phone:e.target.value})} style={inputStyle} onFocus={focus} onBlur={blur}/></Field>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px"}}>
                  <Field label={t("billingAddr")}><input value={customer.customer_address} onChange={e=>setCustomer({...customer,customer_address:e.target.value})} style={inputStyle} onFocus={focus} onBlur={blur}/></Field>
                  <Field label={t("shippingAddr")}><input value={customer.customer_address} onChange={e=>setCustomer({...customer,customer_address:e.target.value})} style={inputStyle} onFocus={focus} onBlur={blur}/></Field>
                </div>
                <Field label={t("email")}><input type="email" value={customer.customer_email} onChange={e=>setCustomer({...customer,customer_email:e.target.value})} style={inputStyle} onFocus={focus} onBlur={blur}/></Field>
              </div>
            </div>
                ...
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:"13px"}}>
                  ...
                  <tbody>
                    ...
                    {items.map((item,i)=>{
                      ...
                      return (
                        <tr key={i} style={{borderBottom:"1px solid var(--border)"}}>
                          <td style={{padding:"8px 6px"}}><input value={item.product_name} onChange={e=>updateItem(i,"product_name",e.target.value)} placeholder={t("product")} style={{...inputStyle,width:"140px",padding:"8px 10px",fontSize:"13px"}} onFocus={focus} onBlur={blur}/></td>
                          <td style={{padding:"8px 6px"}}><input value={item.description} onChange={e=>updateItem(i,"description",e.target.value)} placeholder={t("description")} style={{...inputStyle,width:"160px",padding:"8px 10px",fontSize:"13px"}} onFocus={focus} onBlur={blur}/></td>
                          <td style={{padding:"8px 6px"}}><input type="number" value={item.qty} onChange={e=>updateItem(i,"qty",Number(e.target.value))} style={{...inputStyle,width:"62px",padding:"8px 10px",fontSize:"13px",textAlign:"center"}} onFocus={focus} onBlur={blur}/></td>
                          <td style={{padding:"8px 6px"}}><input type="number" value={item.unit_price} onChange={e=>updateItem(i,"unit_price",Number(e.target.value))} style={{...inputStyle,width:"100px",padding:"8px 10px",fontSize:"13px",textAlign:"right"}} onFocus={focus} onBlur={blur}/></td>
                          <td style={{padding:"8px 6px"}}><input type="number" value={item.discount} onChange={e=>updateItem(i,"discount",Number(e.target.value))} style={{...inputStyle,width:"70px",padding:"8px 10px",fontSize:"13px",textAlign:"center"}} onFocus={focus} onBlur={blur}/></td>
                          <td style={{padding:"8px 6px"}}><select style={{...inputStyle,width:"70px",padding:"8px 8px",fontSize:"12px"}}><option>%</option><option>$</option></select></td>
