...
/* ============================================================
   LANGUAGE CONTEXT – export and use useLang() in every page
   ============================================================ */
...
const dict: Record<string, Record<Lang, string>> = {
  ...
  docsInPipeline:{en:"{n} document{s} in your pipeline", th:"{n} รายการในคิว"},
  ...
  confirmedOrders:{en:"{n} confirmed order{s}",th:"{n} คำสั่งซื้อที่ยืนยัน"},
...
interface LangContextType { lang: Lang; setLang: (l: Lang) => void; t: (key: string, vars?: Record<string,string|number>) => string; }
export const LangContext = createContext<LangContextType>({ lang: "en", setLang: () => {}, t: (k) => k });
export function useLang() { return useContext(LangContext); }

/* ---- nav ---- */
const navItems = [
  { key: "dashboard", href: "/", icon: <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><rect x="3" y="3" width="7" height="7" rx="1.5" /><rect x="14" y="3" width="7" height="7" rx="1.5" /><rect x="3" y="14" width="7" height="7" rx="1.5" /><rect x="14" y="14" width="7" height="7" rx="1.5" /></svg> },
  { key: "quotations", href: "/quotations", icon: <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414A1 1 0 0121 9.414V19a2 2 0 01-2 2z" /></svg> },
  { key: "salesOrders", href: "/sale", icon: <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /></svg> },
];
...
export function SidebarLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("wisdom-lang") as Lang | null;
    ...
    <LangContext.Provider value={{ lang, setLang, t }}>
      <div style={{ display:"flex", minHeight:"100vh", position:"relative" }}>
        ...
        <aside style={{ width:"220px", minHeight:"100vh", position:"fixed", top:0, left:0, bottom:0, zIndex:50, display:"flex", flexDirection:"column", background:"var(--bg-sidebar)", borderRight:"1px solid var(--border)", backdropFilter:"blur(20px)", WebkitBackdropFilter:"blur(20px)", transition:"background 0.4s, border-color 0.4s" }}>
          <div style={{ padding:"24px 20px 20px", display:"flex", alignItems:"center", gap:"11px" }}>
          ...
          <div style={{ margin:"0 16px 16px", height:"1px", background:"var(--border)" }} />
          <div style={{ padding:"0 20px 8px", fontSize:"10px", fontWeight:600, color:"var(--text-muted)", letterSpacing:"0.1em" }}>{t("workspace")}</div>
          <nav style={{ flex:1, padding:"0 10px", display:"flex", flexDirection:"column", gap:"3px" }}>
            {navItems.map(item => {
              const active = isActive(item.href);
              return (
                <button key={item.href} onClick={() => router.push(item.href)} style={{ display:"flex", alignItems:"center", gap:"11px", padding:"10px 14px", borderRadius:"10px", border:"none", cursor:"pointer", fontFamily:"var(--font-body)", fontSize:"13.5px", fontWeight:active?600:500, letterSpacing:"-0.01em", textAlign:"left", width:"100%", transition:"all 0.2s ease", background:active?"linear-gradient(135deg,rgba(124,58,237,0.2),rgba(219,39,119,0.12))":"transparent", color:active?"var(--accent)":"var(--text-secondary)", boxShadow:active?"inset 0 0 0 1px rgba(124,58,237,0.25)":"none", position:"relative" }}
          ...
          <div style={{ margin:"16px", padding:"14px", borderRadius:"12px", background:"linear-gradient(135deg,rgba(124,58,237,0.12),rgba(219,39,119,0.08))", border:"1px solid rgba(124,58,237,0.2)" }}>
        ...
        {/* MAIN */}
        <div style={{ flex:1, marginLeft:"220px", display:"flex", flexDirection:"column", position:"relative", zIndex:1 }}>
          <header style={{ position:"sticky", top:0, zIndex:40, height:"56px", display:"flex", alignItems:"center", justifyContent:"flex-end", padding:"0 28px", background:"var(--bg-sidebar)", borderBottom:"1px solid var(--border)", backdropFilter:"blur(20px)", WebkitBackdropFilter:"blur(20px)", gap:"10px", transition:"background 0.4s, border-color 0.4s" }}>
            {/* Live */}
            <div style={{ display:"flex", alignItems:"center", gap:"6px", padding:"5px 12px", borderRadius:"20px", border:"1px solid rgba(34,197,94,0.3)", background:"rgba(34,197,94,0.1)" }}>
              ...
              <span style={{ fontSize:"11.5px", fontWeight:600, color:"#22c55e", letterSpacing:"0.04em" }}>{t("live")}</span>
            </div>
            {/* Lang toggle */}
            <div style={{ display:"flex", alignItems:"center", borderRadius:"10px", border:"1px solid var(--border)", background:"var(--bg-card)", overflow:"hidden", backdropFilter:"blur(8px)" }}>
              {(["en","th"] as Lang[]).map(l => (
                <button key={l} onClick={() => setLang(l)} style={{ padding:"6px 12px", border:"none", cursor:"pointer", fontFamily:"var(--font-body)", fontSize:"12px", fontWeight:700, letterSpacing:"0.04em", transition:"all 0.2s", background:lang===l?"linear-gradient(135deg,#7c3aed,#db2777)":"transparent", color:lang===l?"white":"var(--text-muted)" }}>{l.toUpperCase()}</button>
              ))}
            </div>
            {/* Theme */}
            <button onClick={toggle} style={{ width:"36px", height:"36px", borderRadius:"50%", border:"1px solid var(--border)", background:"var(--bg-card)", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", color:"var(--text-secondary)", transition:"all 0.25s ease", backdropFilter:"blur(8px)" }}
            ...
            {/* Avatar */}
            <div style={{ width:"34px", height:"34px", borderRadius:"50%", background:"linear-gradient(135deg,#7c3aed,#db2777)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"13px", fontWeight:700, color:"white", boxShadow:"0 0 0 2px var(--bg-sidebar), 0 0 0 3px rgba(124,58,237,0.4)", cursor:"pointer" }}>W</div>
          </header>
