...
app.get("/customers", async (req, res) => {
    try {
        const r = await db_1.pool.query("SELECT * FROM customers ORDER BY customer_id DESC");
        res.json(r.rows);
    }
    catch (err) {
...
app.post("/quotations", async (req, res) => {
    ...
    try {
        ...
        if (!cid) {
            ...
            const c = await client.query(`INSERT INTO customers
        (customer_name, customer_company, customer_phone, customer_address, customer_email)
        VALUES ($1,$2,$3,$4,$5)
        RETURNING customer_id`, [
                customer.customer_name,
        ...
        const total = subtotal + vat;
        /* ===== insert quotation ===== */
        const q = await client.query(`INSERT INTO quotations
      (quotation_no, customer_id, issue_date, expiry_date, subtotal, vat, total, status)
      VALUES ($1,$2,$3,$4,$5,$6,$7,'draft')
      RETURNING *`, [
            `QT-${Date.now()}`,
...
app.get("/quotations/:id", async (req, res) => {
    ...
    `, [id]);
        ...
        const items = await db_1.pool.query("SELECT * FROM quotation_items WHERE quotation_id=$1", [id]);
        res.json({
            ...q.rows[0],
            items: items.rows,
        });
...
app.put("/quotations/:id", async (req, res) => {
    ...
    const { issue_date, expiry_date } = req.body;
    try {
        await db_1.pool.query(`UPDATE quotations 
       SET issue_date=$1, expiry_date=$2 
       WHERE quotation_id=$3`, [issue_date, expiry_date, id]);
        res.json({ message: "updated" });
    }
    catch (err) {
        console.error(err.message);
...
app.put("/quotations/:id/status", async (req, res) => {
    ...
    const { status } = req.body;
    try {
        const r = await db_1.pool.query(`UPDATE quotations 
       SET status=$1 
       WHERE quotation_id=$2 
       RETURNING *`, [status, id]);
        res.json(r.rows[0]);
...
app.delete("/quotations/:id", async (req, res) => {
    const { id } = req.params;
    try {
        await db_1.pool.query("DELETE FROM quotation_items WHERE quotation_id=$1", [id]);
        await db_1.pool.query("DELETE FROM quotations WHERE quotation_id=$1", [id]);
