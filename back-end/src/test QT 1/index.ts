...
app.post("/quotations", async (req, res) => {
  ...
  try {
    ...
    const q = await client.query(
      `INSERT INTO quotations (quotation_no, customer_id, issue_date, expiry_date)
       VALUES ($1,$2,$3,$4)
       RETURNING *`,
      ["QT-" + Date.now(), customer_id, issue_date, expiry_date]
    );
    ...
    await client.query(
      `UPDATE quotations SET subtotal=$1, vat=$2, total=$3 WHERE quotation_id=$4`,
      [subtotal, vat, total, quotation_id]
    );
